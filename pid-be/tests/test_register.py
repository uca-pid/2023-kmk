...
def delete_test_patients():
    yield
    try:
        created_test_users_uid = auth.get_user_by_email(
            a_KMK_patient_information["email"]
        ).uid
        auth.delete_user(created_test_users_uid)
        db.collection("patients").document(created_test_users_uid).delete()
    except:
...
def delete_test_physicians():
    ...
    try:
        ...
        auth.delete_user(created_test_users_uid)
        db.collection("physicians").document(created_test_users_uid).delete()
    except:
...
def test_register_physician_sets_information_object_in_firestore():
    ...
    assert (
        physician_object_from_firestore["matricula"]
        == a_KMK_physician_information["matricula"]
    )
    assert (
